...
class TestModel {
    ...
    public function findAllCat($id) {
        $conn = $this->getDb();
        $sth = $conn->prepare("select * from etx2y_question where testid = :id");
        $sth->execute(array(
            ':id' => $id
        ));
        $result = $sth->fetchAll(PDO::FETCH_OBJ);
    ...
    public function updateOne($item=  array()) {
        $conn = $this->getDb();
        $q = "update $this->table set name=:name,catid=:catid,description= :description  where id=:id";
        $stmt = $conn->prepare($q);
        $stmt->execute($item);
    ...
    public function addOne($item=  array()) {
        unset($item["id"]);
        $conn = $this->getDb();
        $q = "INSERT INTO $this->table (name, catid, description) values (:name, :catid, :description)";
        $stmt = $conn->prepare($q);
        $stmt->execute($item);
    }
